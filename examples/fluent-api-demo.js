...
// Example 8: New system prompt and max turns features
async function systemPromptExample() {
    console.log('\n=== System Prompt and Max Turns Example ===\n')

    // Using system prompt to set behavior
    const response1 = await claude()
        .withSystemPrompt(
            'You are a helpful programming tutor. Always explain concepts clearly.'
        )
        .withMaxTurns(3)
        .skipPermissions()
    ...
    console.log('Programming tutor response:', response1)

    // Appending to system prompt for additional instructions
    const response2 = await claude()
        .withSystemPrompt('You are a creative writer.')
        .appendSystemPrompt('Always write in a humorous style.')
        .withMaxTurns(2)
        .skipPermissions()
        .query('Write a short story about a debugging session')
        .asText()

    console.log('\nCreative writer response:', response2)

    // Combining with other configurations
    const response3 = await claude()
        .withModel('sonnet')
        .withSystemPrompt('You are a concise technical expert.')
        .appendSystemPrompt('Limit all responses to 2 sentences.')
        .withMaxTurns(1)
        .skipPermissions()
        .query('Explain REST APIs')
        .asText()

    console.log('\nConcise expert response:', response3)
}
